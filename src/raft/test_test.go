...
func TestReElection2A(t *testing.T) {
	...
	Debug(dTest, "%v and %v are disconnected", leader2, (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	...
	Debug(dTest, "%v is connected", (leader2+1)%servers)
	cfg.checkOneLeader()
	//fmt.Printf("Fourth check point, the leader is %v\n", leader)

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	Debug(dTest, "%v is connected", leader2)
	cfg.checkOneLeader()
...
func TestFailAgree2B(t *testing.T) {
	...
	Debug(dTest, "S%d is connected", (leader+1)%servers)
...
func TestBackup2B(t *testing.T) {
	...
	Debug(dTest, "S%d, S%d and S%d are disconnected", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	cfg.disconnect((leader1 + 1) % servers)
	Debug(dTest, "S%d and S%d are disconnected", leader1, (leader1+1)%servers)

	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	cfg.connect((leader1 + 3) % servers)
	cfg.connect((leader1 + 4) % servers)
	Debug(dTest, "S%d, S%d and S%d are connected", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	Debug(dTest, "S%d is disconnected", other)
	...
	cfg.connect(other)
	Debug(dTest, "Only S%d, S%d and S%d are connected", leader1, (leader1+1)%servers, other)
	...
	Debug(dTest, "All servers are connected")
	cfg.one(rand.Int(), servers, true)
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			Debug(dTest, "S%d is disconnected", victim)
		...
		if disconnect {
			...
			Debug(dTest, "S%d is connected", victim)
			cfg.connect(victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			...
			cfg.connect(victim)
			Debug(dTest, "S%d crashed and  is connected", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
	}
